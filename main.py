...
app = FastAPI(title="BALESHWAR ULTRA SPAM - Dashboard")
...
def load_users():
    if not os.path.exists(USERS_FILE):
        default_users = {
            "BALESHWAR": {"password": "FATHER", "role": "admin"}
        }
    ...
    try:
        ...
        return {"BALESHWAR": {"password": "FATHER", "role": "admin"}}
...
<title>⚡ BALESHWAR ULTRA SPAM</title>
<style>
    ...
    users.forEach(u => {
      listCont.innerHTML += `
        <div style="background: rgba(255,255,255,0.05); padding: 5px 10px; border-radius:6px; display:flex; align-items:center; gap:10px; border:1px solid rgba(255,255,255,0.1)">
          ...
          ${u.username !== "BALESHWAR" ? `<button style="background:none; border:none; color:red; cursor:pointer;" onclick="deleteUser('${u.username}')">❌</button>` : ''}
        </div>
      `;
    });
  }
